...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const producto = await Producto.findByIdAndUpdate(params.id, data, { new: true, runValidators: true })
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const producto = await Producto.findByIdAndUpdate(params.id, { activo: false }, { new: true })
